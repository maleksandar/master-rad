...
\apstr{%
U okviru ovog rada predstavljen je razvoj okruženja \emph{Pure} - modernog okruženja za razvoj veb aplikacija baziranog na šablonu \emph{Flux}. U narednim poglavljima nalaze se obrazloženja za odabir konkretnih alata i biblioteka tokom implementacije, primeri korišćenja okruženja za pisanje korisničkih aplikacija kao i pojašnjenje implementacionih detalja najvažnijih elemenata okruženja.
}
...
\noindent Ukoliko u pretraživaču otvorimo stranicu \url{http://localhost:1234},
...
Primer jednog funkcionalnog elementa je instanca klase \code{DivElement}. Instance ovih klasa se nikada ne kreiraju direktno pozivanjem konstruktora,
već isključivo korišćenjem specijalnih \emph{factory}-funkcija\footnote{\emph{Factory} - eng. Fabrika. U ovom kontekstu: "Šablon Fabrika" - šablon za kreiranje novih objekata.}.
...
U ovom poglavlju ćemo razmotriti najpre zahteve koje treba da ispuni novo okruženje,
a zatim ćemo se upustiti i u detalje implementacije okruženja \emph{Pure} i
pojasniti neke odluke donete tokom dizajniranja samog okruženja.
...
\begin{enumerate}
  \item Instanciranje korene komponente pozivom prosleđene \emph{factory}\footnote{\emph{Factory} - eng. Fabrika. U ovom konteksu "šablon Fabrika" \cite{GoF}.}-funkcije (Linja 10) \cite{GoF}.
...
Primetimo da drugi argument koji \code{bootsrap()} funkcija prihvata,
...
Kako bismo izgradili nove komponente koristeći šablon kompozicije (eng. \emph{Composite pattern}),
...
Koriščenjem (pojednostavljenog) šablona \emph{dekorator} \cite{GoF} razdvajamo implementaciju za osluškivanje i
...
u šablonu \emph{Observer}\footnote{\emph{Observer} - eng. Posmatrač} \cite{GoF}. Drugim rečima, nad objektom tipa
...
\begin{enumerate}
  ...
  \item Svojstvo za vraćanje trenutnog stanja (\code{get state()}).
...
To nam garantuje da osnovni princip šablona \emph{Flux} neće biti narušen.

\section{Memoizacija}
Već smo napomenuli da se komponente nikada ne instanciraju direktno preko svojih konstruktora,
već isključivo preko \emph{factory}-funkcija koje možemo napraviti koristeći ugrađenu funkciju \code{componentFactory}
koju možemo uvesti sa putanje \code{"pure-framework/core"}. Pogledajmo detalje implementacije ove funkcije:
...
\begin{enumerate}
  \item Funkciju za izračunavanje trenutnog stanja (\code{state: () => ModelType}).
...
su prisutni u popularnim okruženjima za razvoj klijentskih veb aplikacija. Kao takavo, više predstavlja dokaz koncepta (eng. \emph{Proof of Concept}), nego rešenje spremno za komercijalnu upotrebu.
